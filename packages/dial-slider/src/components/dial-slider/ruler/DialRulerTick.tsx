...
const { TICK_SPACING, TICK_HEIGHT, CENTER_TICK_HEIGHT, TICK_WIDTH } =
  DIAL_CONFIG;
...
export const DialRulerTick = React.memo(function DialRulerTick({
  ...
  useAnimatedReaction(
    () => {
      const currentValue = -translationX.get() / TICK_SPACING;
      const nearestTickVal = getNearestDialTick(currentValue, tickValues);
      return tickValue === nearestTickVal;
    },
    (isNearest, wasNearest) => {
      if (isNearest !== wasNearest) {
        heightAnim.set(
          withTiming(isNearest ? CENTER_TICK_HEIGHT : TICK_HEIGHT, {
            duration: 150,
  ...
  useAnimatedReaction(
    () => Math.round(-translationX.get() / TICK_SPACING),
    (currentVal, prevVal) => {
  ...
  const animStyle = useAnimatedStyle(() => {
    const x = tickX + translationX.get();
    const opacity = getDialTickOpacity(x, screenCenter);
    const startVal = -dragStartX.get() / TICK_SPACING;
    const currentVal = -translationX.get() / TICK_SPACING;
    ...
    return {
      height,
      opacity,
      backgroundColor: isNearest ? COLORS.POSITIVE : `rgb(${r},${g},${b})`,
