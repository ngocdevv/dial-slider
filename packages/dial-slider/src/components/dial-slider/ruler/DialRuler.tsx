...
const { CENTER_TICK_HEIGHT, TICK_STEP, MAJOR_TICK_EVERY } = DIAL_CONFIG;
...
export function DialRuler({
  ...
  const tickValues = useMemo(
    () =>
      buildDialTickValues(minValue, maxValue, TICK_STEP, MAX_RENDERED_TICKS),
    [minValue, maxValue]
  );
  const ticks = useMemo(
    () =>
  ...
  const { contentStyle, gesture, isDragging, startX, translationX } =
    useDialRulerMotion({
      value,
      initialValue,
      minValue,
