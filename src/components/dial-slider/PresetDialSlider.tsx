...
import { createCircularArcPath, getSignedProgressColor } from './ring-math';

const ITEM_SIZE = 58;
const ITEM_GAP = 12;
const RING_CENTER = ITEM_SIZE / 2;
const RING_RADIUS = RING_CENTER - DIAL_CONFIG.RING_STROKE_WIDTH / 2;
const VALUE_BADGE_DELAY_MS = 650;
...
function getInitialValue(preset: DialPreset) {
    ...
    const requestedInitial = Number.isFinite(preset.initialValue)
        ...
        : 0;
    return clamp(requestedInitial, min, max);
}

function getProgress(value: number, min: number, max: number) {
    if (value >= 0) return max <= 0 ? 0 : Math.min(value / max, 1);
    return min >= 0 ? 0 : Math.min(Math.abs(value) / Math.abs(min), 1);
}
...
export interface PresetDialSliderProps {
...
function PresetRing({
    ...
    const progress = getProgress(value, minValue, maxValue);
    ...
    return (
        ...
        >
            <Circle
                ...
                strokeWidth={DIAL_CONFIG.RING_BG_STROKE_WIDTH}
            />
            {arcPath ? (
                <Path
                    d={arcPath}
                    fill="none"
                    stroke={color}
                    strokeWidth={DIAL_CONFIG.RING_STROKE_WIDTH}
                    strokeLinecap="round"
                />
            ) : null}
        </Svg>
...
function PresetButton({
    ...
    const { min, max } = getRange(preset);
    const adjusted = Math.round(value) !== Math.round(clamp(0, min, max));
...
export function PresetDialSlider({
    presets: presetInput,
    initialPresetId,
    onPresetChange,
    onValueChange,
    ...
    const handleSharedValueChange = useCallback(
        (nextValue: number, callbackRevision: number) => {
            ...
            const { min, max } = getRange(preset);
            const next = clamp(Math.round(nextValue), min, max);
    ...
    const handleInteractionStart = useCallback(() => {
        ...
        presetRowOpacity.value = withTiming(0.06, { duration: 140 });
    ...
    const contentPadding = Math.max(0, (viewportWidth - ITEM_SIZE) / 2);
